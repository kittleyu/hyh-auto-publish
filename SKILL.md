...
| 当前默认公司 | 账号活跃公司（如 `<CORP_ID>=<客户公司名>`）。可用 `CORP_ID` 环境变量切换任意可达公司 |
...
| 媒体热度指标 | `media.quote_cnt` = 平台内置「AI 引用次数」。⚠️ **实测基本是坏的**：某客户 corp 的 100 个自媒体仅 2 个 >0（都是「有驾」=0.02，单价却最贵 10864）。按 quote_cnt 排序会退化成「选最贵媒体」，**成本优先用 `--sort price`** |
...
> 以下记录已去敏（客户名/文章标题以角色占位），保留的是**可复用的技术经验**。

- **[游戏行业客户 A]**（corp 新/旧两个 id）：20 篇待审核 → audit_run 审核 20/20 → `--sort price --top-k 5` 分散铺量，成本 25608 智豆（原热度方案 16.1 万，因 quote_cnt 坏数据退化）。
- **[医疗行业客户 B]**（静脉曲张方向）：10 篇待发布（最新 4 篇已 `pub=3` 自动排除）→ 5 个医疗历史媒体轮转 → 其中 2 个**已被平台禁用**，改用人民网健康、光明医疗网补齐 → 10/10 成功，共 19600 智豆。
  - 关键教训：**历史媒体会被平台禁用**，多发前务必小批量试或接受失败重试。
- **[期货行业客户 C]**（期货开户方向）：多个同名 corp 中仅 1 个有文章（其余为空壳）。锚定某篇待发布文章标题恰为列表最新一篇；取最新 5 篇（均为 `pub=0&audit=1`），从历史记录挑 5 个金融平台（雪球/东方财富/新浪/知乎/百家号）各 1 篇。API 首发因「账户余额不足」失败（后用户确认**智豆不限量**，疑瞬时异常）；用户手动发 3 篇、脚本补发 2 篇，5 篇全部进入发布流程（3 已发布 + 2 发布中）。
  - 修正：用户明确智豆不限量；默认候选源已改 `history`（即后台「历史记录」面板）；选媒体以主题相关/权威优先，不为省钱妥协。
...
- 同一账号下多个公司可能有**重复公司名**（如某客户有 `CORP_NEW` / `CORP_OLD` 两个 corp_id），`companies.txt` 里查 corp_id 时以数字为准，必要时跟用户确认用哪个。
